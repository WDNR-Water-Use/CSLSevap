...
for (lake in lakes){
  ...
  for (i in 1:length(methods)) {
    ...
    monthly_E <- daily_met %>%
                 ...
                 ungroup() %>%
                 select(.data$date, .data$E_mm)
    this_E    <- select(monthly_E, "E_mm")
    dates     <- select(monthly_E, "date")
    lakes     <- rep(lake, nrow(monthly_E))
    colnames(this_E) <- names[i]
    
    lake_E           <- as.data.frame(c(lake_E, this_E))
  }
  lake_E <- cbind(lake_E, dates, lakes)
  all_E  <- rbind(all_E, lake_E)
}
all_E     <- all_E %>% filter(date != mdy("06-01-2018"))
melted_E  <- melt(all_E, id.vars = c("date", "lakes"))
```
